...
class PrintPreviewActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        repo = TrisquelRepo(application)

        CoroutineScope(Dispatchers.Main).launch {
            val sb = withContext(Dispatchers.IO) {
                val filmrollEntity = repo.getFilmRollRaw(id) ?: return@withContext null
                val photosEntities = repo.getPhotosByFilmRollIdRaw(id)
                val cameraEntity = repo.getCamera(filmrollEntity.camera ?: 0) ?: return@withContext null
                val camera = CameraSpec.fromEntity(cameraEntity)
                
                name = filmrollEntity.name
                ...
                sb.append("<tr><td>${getString(R.string.label_camera)}</td>      <td>${camera.manufacturer} ${camera.modelName}</td></tr>")
                sb.append("<tr><td>${getString(R.string.label_manufacturer)}</td> <td>${filmrollEntity.manufacturer}</td></tr>")
                sb.append("<tr><td>${getString(R.string.label_brand)}</td>       <td>${filmrollEntity.brand}</td></tr>")
                sb.append("<tr><td>${getString(R.string.label_iso)}</td>         <td>${filmrollEntity.iso}</td></tr>")
                sb.append("</table></div><div class=\"content\">")

                for (entity in photosEntities) {
                    val p = Photo.fromEntity(entity)
                    val accessories = p.accessories.map { a -> repo.getAccessory(a)?.name ?: "" }.filter { it.isNotEmpty() }.joinToString(", ")
                    sb.append("<div class=\"photo\">")
                    sb.append("<h2>#${p.frameIndex + 1}</h2>")
                    sb.append("<table width=\"100%\">")
                    val lensEntity = repo.getLens(p.lensid)
